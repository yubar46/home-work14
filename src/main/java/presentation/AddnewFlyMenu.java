...
public class AddnewFlyMenu implements Menu {
  ...
  public Menu action() {
    ...
    Fly fly = new Fly(flyCost,airplaneModel,sourceCity,destinationCity,localTime,localDate);
    fly.setAirLine(ApplicationContext.getInstance().getAirLine());
    ApplicationContext.Instance.getFlyService().create(fly);
    return new AirlineInsideMenu();
  }
}
